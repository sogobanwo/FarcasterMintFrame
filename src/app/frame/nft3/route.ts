...
export async function GET(req: NextRequest, res: NextResponse) {
    try {
        const frameMetadata = fdk.getFrameMetadata({
            image: { url: "https://amaranth-genuine-kangaroo-139.mypinata.cloud/ipfs/QmfJVQk2nszevbyEGJntcVAciKGEGQrBJR4gCWb78hBqi9/3.jpeg" },
            post_url: `${process.env.BASE_URL}/frame/nft3`,
...
export async function POST(req: NextRequest, res: NextResponse) {
    ...
    const balance = await balanceOf(address, 3);
    console.log(balance);
    if (typeof balance === "number" && balance !== null && balance < 1) {
        try {
            const mint = await mintNft(address, 3);
            console.log(mint);
            const frameMetadata = fdk.getFrameMetadata({
